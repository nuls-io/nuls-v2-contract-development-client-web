              ...
              <i class="el-icon-edit-outline click"></i>
                ...
                <el-table-column :label="$t('address.address5')" align="center" width="350">
                    <template slot-scope="scope">
                        ...
                        <label class="click tab_bn" @click="selectionAddress(scope.row)"
                               v-else>{{$t('public.into')}}</label>
                    </template>
                </el-table-column>
            </el-table>
            <div class="pages">
    ...
    export default {
        ...
        methods: {
                ...
                await axios.post(LOCALHOST_API_URL, PARAMETER)
                    .then(async (response) => {
                        if (response.data.hasOwnProperty("result")) {
                            ...
                            this.$message({
                                message: this.$t('contract.contract11') + response.data.error.message,
                                type: 'error',
                                duration: 1000
                            });
                        }
            ...
            deleteAddress(rowInfo) {
                this.selectAddressInfo = rowInfo;
                if (this.selectAddressInfo.encrypted) {
            ...
            selectionAddress(rowInfo) {
                localStorage.setItem(chainIdNumber(), JSON.stringify(rowInfo));
                // this.getAddressList();
                this.$router.push({
                    name: 'home',
                })
            ...
            passSubmit(password) {
                ...
                axios.post(LOCALHOST_API_URL, PARAMETER)
                    .then((response) => {
                        if (response.data.hasOwnProperty('result')) {
                            let info = localStorage.getItem(chainIdNumber());
                            if (info && info.address && this.selectAddressInfo.address == info.address) {
                            ...
                            this.$message({
                                message: this.$t('address.address15') + response.data.error.message,
                                type: 'error',
                                duration: 1000
                            });
                        }
